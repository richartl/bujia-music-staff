    ...
    <main className="mx-auto max-w-3xl space-y-4 p-3 pb-8 sm:p-4">
      ...
      <section className="rounded-2xl border border-slate-300 bg-white p-4 shadow-sm">
        <div className="flex items-center justify-between gap-2">
          <h2 className="text-base font-semibold text-slate-900">Servicios y avance</h2>
          <span className="rounded-full bg-slate-100 px-2 py-1 text-[11px] font-semibold text-slate-700">
            {(data.services || []).length} servicio(s)
          </span>
        </div>
        {(data.services || []).length === 0 ? (
          <div className="mt-3 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4 text-center text-sm text-slate-600">
            Aún no hay servicios visibles para esta orden.
          </div>
        ) : (
      ...
      <section className="rounded-2xl border border-slate-300 bg-white p-4 shadow-sm">
        <h2 className="text-xs font-semibold uppercase tracking-wide text-slate-600">Resumen económico</h2>
        <p className="mt-1 text-xs text-slate-500">Montos informativos de tu orden.</p>
        <div className="mt-2 grid grid-cols-2 gap-2">
          ...
          <MoneyPill label="Refacciones" value={totals.partsTotal} />
...
function MoneyPill({ label, value, emphasized = false }: { label: string; value: number; emphasized?: boolean }) {
  return (
    <div className={`rounded-xl border px-2.5 py-2 ${emphasized ? 'border-amber-300 bg-amber-50' : 'border-slate-200 bg-slate-50/70'}`}>
      <p className={`text-[11px] font-medium uppercase tracking-wide ${emphasized ? 'text-amber-700' : 'text-slate-600'}`}>{label}</p>
      <p className={`mt-0.5 text-sm font-semibold ${emphasized ? 'text-amber-800' : 'text-slate-900'}`}>{currency(value)}</p>
    </div>
  );
}
